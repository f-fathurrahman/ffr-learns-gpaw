# Originally from gpaw/atom/radialgd.py
...
def my_radial_grid_descriptor(eq: str, **kwargs) -> 'MyRadialGridDescriptor':
    if eq == 'r=d*i':
        assert int(kwargs['istart']) == 0
        return MyEquidistantRadialGridDescriptor(float(kwargs['d']),
                                               int(kwargs['iend']) + 1)
    if eq == 'r=a*i/(n-i)':
        beta = float(kwargs['a'])
        ng = int(kwargs['n'])
        return MyAERadialGridDescriptor(beta / ng, 1.0 / ng, ng)
    if eq == 'r=a*i/(1-b*i)':
        a = float(kwargs['a'])
        b = float(kwargs['b'])
        N = int(kwargs['n'])
        return MyAERadialGridDescriptor(a, b, N)
    if eq == 'r=a*(exp(d*i)-1)':
        a = float(kwargs['a'])
        d = float(kwargs['d'])
        N = int(kwargs['iend']) + 1
        return MyAbinitRadialGridDescriptor(a, d, N)
    raise ValueError('Unknown grid: ' + eq)


def my_fsbt(l, f_g, r_g, G_k):
...
class MyRadialGridDescriptor(ABC):
...
class MyAERadialGridDescriptor(MyRadialGridDescriptor):
    def __init__(self, a, b, N=1000, default_spline_points=25):
        ...
        MyRadialGridDescriptor.__init__(self, r_g, dr_g, default_spline_points)
    ...
    def new(self, N):
        return MyAERadialGridDescriptor(self.a, self.b, N)
...
class MyAbinitRadialGridDescriptor(MyRadialGridDescriptor):
