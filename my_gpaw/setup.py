...
from my_gpaw.gaunt import gaunt, nabla
from my_gpaw.overlap import OverlapCorrections
from my_gpaw.rotation import rotation
from my_gpaw.setup_data import SetupData, search_for_file
from my_gpaw.spline import Spline
from my_gpaw.utilities import pack, unpack
from my_gpaw.xc import XC
from my_gpaw.new import zip
from my_gpaw.xc.ri.spherical_hse_kernel import RadialHSE
...
def create_setup(symbol, xc='LDA', lmax=0,
    ...
    if isinstance(type, str) and ':' in type:
        from my_gpaw.hubbard import parse_hubbard_string
    ...
    if setupdata is None:
        if type == 'hgh' or type == 'hgh.sc':
            lmax = 0
            from my_gpaw.hgh import HGHSetupData, sc_setups, setups
            if type == 'hgh.sc':
                table = sc_setups
            else:
                table = setups
            ...
            from my_gpaw.ah import AppelbaumHamann
            ah = AppelbaumHamann()
            ah.build(basis)
            return ah
        elif type == 'ae':
            from my_gpaw.ae import HydrogenAllElectronSetup
            assert symbol == 'H'
            ae = HydrogenAllElectronSetup()
            ae.build(basis)
            return ae
        elif type == 'ghost':
            from my_gpaw.lcao.bsse import GhostSetupData
            setupdata = GhostSetupData(symbol)
        elif type == 'sg15':
            from my_gpaw.upf import read_sg15
...
class Setups(list):
    ...
    def estimate_dedecut(self, ecut):
        from my_gpaw.utilities.ekin import dekindecut, ekin
        dedecut = 0.0
        e = {}
        for id in self.id_a:
            if id not in e:
