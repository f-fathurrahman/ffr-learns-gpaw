...
class GPAWLogger:
    ...
    def header(self):
        ...
        self(' |   |   |_  | | | | ')
        self(' | | | | | . | | | | ')
        self(' |__ |  _|___|_____| ', my_gpaw.__version__)
        self(' |___|_|             ')
        self()
    ...
    def close(self):
        if my_gpaw.dry_run or self._fd.closed:
            return
...
def write_header(log, world):
    ...
    # GPAW
    line = os.path.dirname(my_gpaw.__file__)
    githash = search_current_git_hash(my_gpaw, world)
