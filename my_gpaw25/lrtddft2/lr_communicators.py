...
class LrCommunicators:
    def __init__(self, world, dd_size: int, eh_size: int = None):
        ...
        ----------
        world
          MPI parent communicator (usually ``my_gpaw25.mpi.world``)
        dd_size
        ...
        For 8 MPI processes::
          ...
          lr_comms = LrCommunicators(my_gpaw25.mpi.world, 4, 2)
    ...
    def initialize(self, calc):
        if self.parent_comm is None:
            if calc is not None:
                ...
                self.eh_comm = my_gpaw25.mpi.serial_comm
            else:
                self.parent_comm = my_gpaw25.mpi.serial_comm
                self.dd_comm = my_gpaw25.mpi.serial_comm
                self.eh_comm = my_gpaw25.mpi.serial_comm
        else:
