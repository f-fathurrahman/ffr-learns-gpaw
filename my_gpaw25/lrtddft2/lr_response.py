...
from my_gpaw25.lrtddft2.ks_singles import KohnShamSingleExcitation
from my_gpaw25.lrtddft2.lr_layouts import LrTDDFPTSolveLayout
...
class LrResponse:
    ...
    def get_response_coefficients(self, units='eVang'):
        ...
        if units == 'au':
            ...
            raise RuntimeError(
                'Error in get_response_coefficients: Invalid units.')
    ...
    def get_response_data(self, units='eVangcgs'):
        ...
        data = np.zeros((len(self.lrtddft2.ks_singles.kss_list),
                         2 + 2 + 2 + 2 + 3 + 3 + 3))
    ...
    def get_transition_contribution_maps(self,
                                         occ_min_energy,
                                         occ_max_energy,
                                         unocc_min_energy,
                                         unocc_max_energy,
        ...
        for (ei, ep, a, s, r) in zip(eps_i, eps_p, a_ip, s_ip, r_ip):
            gx = np.exp((-.5 / width / width) *
                        np.power(wx - ei, 2)) / width / np.sqrt(2 * np.pi)
            gy = np.exp((-.5 / width / width) *
                        np.power(wy - ep, 2)) / width / np.sqrt(2 * np.pi)
        ...
        if units == 'au':
            ...
            raise RuntimeError(
                'Error in get_transition_contribution_maps: Invalid units.')
    ...
    def get_approximate_electron_and_hole_densities(self,
                                                    units='au',
                                                    collect=False,
                                                    amplitude_filter=1e-4):
        # Init pair densities
        dnt_Gip = self.lrtddft2.calc.wfs.gd.empty()
        dnt_gip = self.lrtddft2.calc.density.finegd.empty()
        drhot_gip = self.lrtddft2.calc.density.finegd.empty()
        ...
        for k in range(len(f_n)):
            if (abs(fh_n[k]) < amplitude_filter * maxC * maxC
                    and abs(fe_n[k]) < amplitude_filter * maxC * maxC):
                continue
        ...
        for (ip, kss_ip) in enumerate(self.lrtddft2.ks_singles.kss_list):
            ...
            for (jq, kss_jq) in enumerate(self.lrtddft2.ks_singles.kss_list):
                ...
                if (kss_ip.occ_ind == kss_jq.occ_ind
                        and kss_ip.unocc_ind == kss_jq.unocc_ind):
                    continue
                ...
                if (kss_ip.occ_ind == kss_jq.occ_ind
                    ...
                    drhot_ge += (C_im[ip] * C_im[jq] * np.sqrt(
                        kss_ip.pop_diff * kss_jq.pop_diff)) * drhot_gip
                ...
                if (kss_ip.occ_ind != kss_jq.occ_ind
                    ...
                    drhot_gh -= (C_im[ip] * C_im[jq] * np.sqrt(
                        kss_ip.pop_diff * kss_jq.pop_diff)) * drhot_gip
    ...
    def solve_parallel(self):
        ...
        # create BLACS layout
        layout = LrTDDFPTSolveLayout(self.sl_lrtddft, nrows,
                                     self.lrtddft2.lr_comms)
        if (nrows <
